...
# depchase - dependency chasing tool
# Attribute: https://github.com/fedora-modularity/depchase
    ...
    def load_ext(self, repodata):
        ...
        if ext == "FL":
            self.handle.add_rpmmd(f, "FL",
                                  solv.Repo.REPO_USE_LOADING | solv.Repo.REPO_EXTEND_SOLVABLES | solv.Repo.REPO_LOCALPOOL)
...
def solve_a_package(solver, pkg: solv.XSolvable, selfhost=False):
    pool = solver.pool

    problems = solver.solve([(pkg, solv.Job.SOLVER_INSTALL | solv.Job.SOLVER_SOLVABLE)])
    ...
    srcs_done = set()
    while candq:
        jobs = [pool.Job(solv.Job.SOLVER_INSTALL | solv.Job.SOLVER_SOLVABLE | solv.Job.SOLVER_WEAK, p.id) for p in
                candq]
        solver.solve(jobs)
        # print_transaction(pool, solver.transaction())
        ...
        pool.setpooljobs(
            pool.getpooljobs() + [pool.Job(solv.Job.SOLVER_FAVOR | solv.Job.SOLVER_SOLVABLE, p.id) for p in newpkgs])
...
def pkgs_by_sourcepkg(ctx, sourcepkg):
    ...
    sel = pool.select(sourcepkg,
                      solv.Selection.SELECTION_NAME | solv.Selection.SELECTION_DOTARCH | solv.Selection.SELECTION_WITH_SOURCE)
    if not sel.isempty():
        found = sel.solvables()
        assert len(found) == 1, "More matching solvables were found, {}".format(found)
