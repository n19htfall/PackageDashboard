...
class NPMRegistryClient:
    ...
    def get_package_info(
        ...
        try:
            ...
            logging.warning(f"Failed to fetch package info for {package_name}: {e}")
            return None
...
class DatabaseManager:
    ...
    def _extract_repo_url(
        ...
        # 如果没有有效的仓库URL，尝试从NPM注册表获取
        if not _is_vcs_repo_url(repo_url) or not _is_vcs_repo_url(homepage_url):
            npm_info = self.npm_client.get_package_info(package_url.name, package_url.namespace)
            if npm_info:
    ...
    def _extract_license(self, package_json: Dict[str, Any]) -> Optional[str]:
        ...
        if license_spdx is None:
            ...
            if license_detections:
                license_spdx = license_detections[0].get("license_expression_spdx")
        return license_spdx
    ...
    def _extract_dependencies(
        ...
        for dep_name in required_deps:
            version = self._find_dependency_version(dep_name, lock_packages, lock_dependencies)
            if version:
    ...
    async def _save_dependency_relationships(
        ...
        for dep_name, version in dependencies:
            dep_name_parsed, dep_namespace = PackageParser.parse_namespace(dep_name)
            dep_purl = PackageURL(
    ...
    async def _upsert_dependency(self, dependency: PackageDependency) -> None:
        ...
        try:
            ...
            logging.info(f"Dependency {dependency.dep_purl} for {dependency.purl} saved")
        except DuplicateKeyError:
            ...
            logging.info(f"Dependency {dependency.dep_purl} for {dependency.purl} updated")
...
class PackageProcessor:
    ...
    async def process_package(self, package_file: str, save_to_db: bool = True) -> None:
        ...
        temp_files = []
        try:
            logging.info(f"Processing package: {package_file}")

            # 解压和准备文件
            package_path, temp_files = await self._prepare_package(package_file)

            # 运行scancode分析
            SCANCODE_OUTPUT_P = await self._run_scancode_analysis(package_file, package_path)

            # 解析包信息
            package_url, package_json = await self._parse_package_info(SCANCODE_OUTPUT_P)

            if package_url and save_to_db:
                # 保存包信息和依赖关系
                await self.db_manager.save_package(package_url, package_json)
    ...
    async def _run_scancode_analysis(self, package_file: str, package_path: str) -> str:
        ...
        output_json_path = os.path.join(self.config.output_json_dir, f"{filename}.json")
    ...
    async def _parse_package_info(
        ...
        try:
            ...
            for package_info in output_json.get("packages", []):
                if package_info.get("type") == "maven":
            ...
            logging.error(f"Failed to parse package info from {output_json_path}: {e}")
            return None, None
